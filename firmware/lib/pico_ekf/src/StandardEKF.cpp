...
void StandardEKF::init(const EKFConfig& cfg) {
    config = cfg;
    ...
    // Quaternion process noise should be proportional to gyro noise
    // The factor 0.25 comes from the quaternion kinematic equation
    for (int i = 0; i < 4; i++) {
...
void StandardEKF::reset() {
    ...
    P.setIdentity();
    for (int i = 0; i < 4; i++) {
        P(i, i) = 0.01f;  // Initial quaternion uncertainty
    }
    for (int i = 4; i < 7; i++) {
        P(i, i) = 0.01f;  // Initial bias uncertainty (rad/s)^2
    }
    
    EKF_DEBUG_PRINT("EKF reset\n");
}

void StandardEKF::update(const IMUMeasurement& measurement) {
    if (!initialized) {
...
void StandardEKF::predictState(const IMUMeasurement& meas) {
...
void StandardEKF::updateState(const IMUMeasurement& meas) {
...
void StandardEKF::computeJacobians(Matrix& F, const Vector3& omega, float dt) {
    F.setIdentity();
    ...
    // Since omega = omega_measured - bias, the derivatives are negative
    F(0, 4) = -halfDt * q.x;
    F(0, 5) = -halfDt * q.y;
    F(0, 6) = -halfDt * q.z;
...
void StandardEKF::computeMeasurementJacobian(Matrix& H, const Quaternion& q) {
    // Clear H matrix
...
void StandardEKF::getEulerAngles(float& roll, float& pitch, float& yaw) const {
